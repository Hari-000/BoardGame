pipeline {
    ...
    environment {
        ...
        DOCKER_IMAGE = "hari0000/boardshack:${env.BUILD_NUMBER}" // Image name with versioning
    }
    stages {
        stage('Git Checkout') {
            steps {
                git branch: 'prod', credentialsId: 'git-cred', url: 'https://github.com/Hari-000/BoardGame.git'
            }
        }
        stage('Versioning') {
    steps {
...
steps {
withKubeConfig(caCertificate: '', clusterName: 'my-cluster', contextName: '', credentialsId: 'k8-cred', namespace: 'webapps', restrictKubeConfigAccess: false, serverUrl: 'https://F459F278CF0DD1360F535DF7210F18EB.gr7.us-east-2.eks.amazonaws.com') {
